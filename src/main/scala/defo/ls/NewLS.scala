...
import srte.util.Topology
import srte.util.SegmentDB
import srte.util.Demand
import srte.benchmark.Results
import srte.benchmark.Solver

class NewLS(topology: Topology, capacities: Array[Double], ecmps: SegmentDB, demands: Array[Demand], verbous: Boolean, greedy: Boolean) extends Solver {

  private[this] val rand = new Random(0)

  private[this] var alpha = 8.0
  private[this] val beta = 2.0
  ...
  private[this] val invCapa = capacities.map(c => 1 / c) // faster to use
  ...
  def getBestFlows = bestFlows

  override def solve(timeLimit: Long, objective: Double): Results = {
    initialSolution()
    updateMax()
  ...
  private def updateMax(): Unit = {
    ...
    while (e > 0) {
      e -= 1
      val usage = bestFlows(e) * invCapa(e)
      if (usage > bestUsage) {
        bestUsage = usage
      }
    }
  ...
  private def selectEdge(): Int = {
    if (!greedy) edgeTree.get(rand.nextDouble() * edgeTree.totalWeight)
    else selectMaxEdge()
  }
  ...
  private def selectMaxEdge(): Int = {
    ...
    while (e > 0) {
      e -= 1
      val load = bestFlows(e) * invCapa(e)
      if (maxLoad < load) {
        maxEdge = e
        maxLoad = load
      }
  ...
  @inline private def restoreState(): Unit = {
  ...
  @inline private def clearState(): Unit = {
    while (nStoredEdges > 0) {
      nStoredEdges -= 1
      val edge = storedEdges(nStoredEdges)
      changed(edge) = false
  ...
  @inline private def saveState(edge: Int): Unit = {
    if (!changed(edge)) {
      changed(edge) = true
      storedFlows(edge) = bestFlows(edge)
      storedEdges(nStoredEdges) = edge
      nStoredEdges += 1
    }
  }
  ...
  @inline private def resetEdges(): Unit = {
    var e = nEdges
    while (e > 0) {
      e -= 1
      bestFlows(e) = 0
